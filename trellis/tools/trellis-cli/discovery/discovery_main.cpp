...
int discovery_main(int argc, char* argv[]) {
  ...
  HandlersMap handlers{
      {"dump", {discovery_dump_command_desc.data(), [argc, argv]() { return cli::discovery_dump_main(argc, argv); }}},
  };
  if (subcommand.empty()) {
    std::cout << "Must specify a subcommand... " << std::endl;
    cli::PrintCommandsHelp("discovery", handlers);
    return 0;
  }
  return cli::RunCommand("discovery", subcommand, handlers);
}
